...
3. **Schedule the resume — only if there is work to resume.** Decide this first. A cron
   ...
   clears whatever is left. Otherwise — local, session-only, minute-accurate, single fire. **First clear the previous one:** if `resume-<session_id>.json` already exists, `CronDelete` the `resume_cron_id` it names before scheduling anything. A stand-down that was cancelled and then re-entered leaves that job alive, and overwriting the checkpoint loses the only id that could remove it — which is how one session ends up with two resume crons firing at the same minute. Do the scheduling **before** the checkpoint so the new cron's id goes into it in one write (no second edit). Read `wake_at_iso` from **this session's stand-down marker** `standdown-<session_id>.json` (local ISO, e.g. `2026-07-18T01:01:00`; it carries the deferral push — do NOT re-read a fresh guard.sh verdict, which would drop it) and map it straight to a 5-field cron `minute hour day-of-month month day-of-week`, adding ~1 min so it fires just AFTER the reset (day-of-week stays `*`). **Don't eyeball the format — call it exactly like this** (worked example for `wake_at_iso` = `2026-07-18T01:01:00` → fire 01:02):
